...
/* ===== D1 schema helper (added) =====
   This block ensures the D1 database has a suitable `bars` table with a `tf` column.
   It runs at runtime if `env.GSX_DB` is provided. It uses CREATE TABLE IF NOT EXISTS
   so it is safe to call multiple times. The table definition is intentionally simple
   and matches the `d1Bars()` usage in this worker: (t, tf, o, h, l, c, v).
*/
async function ensureD1Schema(env){
  try{
    if (!env || !env.GSX_DB) return { ok:false, reason:'no_db' };
    // Create table if missing (idempotent)
    const sql = `
      CREATE TABLE IF NOT EXISTS bars (
        t INTEGER PRIMARY KEY,
        tf INTEGER DEFAULT 1440,
        o REAL,
        h REAL,
        l REAL,
        c REAL,
    ...
    `;
    // create a simple index to help queries by tf and t
    const sqlIndex = `CREATE INDEX IF NOT EXISTS idx_bars_tf_t ON bars(tf, t);`;
    await env.GSX_DB.prepare(sql).run();
    await env.GSX_DB.prepare(sqlIndex).run();
    return { ok:true };
  }catch(e){
    // don't throw to avoid worker failing; return structured info
    return { ok:false, reason:String(e?.message||e) };
  }
}

/* Call ensureD1Schema at the start of a fetch request (only once per worker invocation is fine).
   The code below tries to run it asynchronously but doesn't block the response heavily.
*/
const _D1_schema_checked = new WeakSet();
async function maybeEnsureD1(env){
  try{
    if (!env || !env.GSX_DB) return;
    // use env object as a key in WeakSet so we run per instance at most once
    if (_D1_schema_checked.has(env)) return;
    const r = await ensureD1Schema(env);
    // mark checked even if it failed to avoid repeating noisy attempts
    _D1_schema_checked.add(env);
    if (!r.ok) console.warn('ensureD1Schema failed:', r.reason);
  }catch(err){
    console.warn('maybeEnsureD1 error', String(err));
  }
}

// At the top of the fetch handler (before routes) call maybeEnsureD1(env);
// NOTE: the original fetch function should be modified to call maybeEnsureD1(env);
// If you cannot or prefer not to edit the fetch body, this helper still exists and
// can be invoked manually from your fetch implementation.
/* ===== end D1 schema helper ===== */
