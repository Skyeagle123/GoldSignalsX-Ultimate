...
// ===== GSX Minimal Additions: normalize TF + unified fetch handler (non-breaking) =====
(function(){
  function normalizeTF(tfRaw){
    try {
      let tf = String(tfRaw || '').trim().toLowerCase();
      tf = tf.replace(/\s+/g,'').replace('يوم','d1').replace('د1','d1').replace('day','d1');
      const map = {
        '1':'1m','1m':'1m',
        '5':'5m','5m':'5m',
        '15':'15m','15m':'15m',
        '30':'30m','30m':'30m',
        '60':'60m','1h':'60m','60m':'60m',
        '240':'240m','4h':'240m','240m':'240m',
        'd1':'1d','1d':'1d'
      };
      return map[tf] || tf;
    } catch(e){ return '1m'; }
  ...
  async function fetchBars(){
    try{
      const baseInput = document.querySelector('#workerUrl') || document.querySelector('[name="workerUrl"]');
      let base = (baseInput ? baseInput.value : (window.GSX_WORKER_URL || '')).trim();
      base = base.replace(/\/+$/,'');
      const limitInput = document.querySelector('#limit') || document.querySelector('[name="limit"]');
      const limit = parseInt(limitInput ? limitInput.value : (window.GSX_LIMIT || 1200),10) || 1200;

      // try active TF button or any selected option
      const activeBtn = document.querySelector('[data-tf].active') ||
                        Array.from(document.querySelectorAll('button')).find(b => b.classList.contains('active') && /^(?:1|5|15|30|60|240|يوم|د\s*1)$/i.test((b.textContent||'').trim())) ||
                        null;
      let rawTf = (activeBtn && (activeBtn.dataset.tf || activeBtn.textContent)) ||
                  (document.querySelector('#tf') && document.querySelector('#tf').value) ||
                  '1m';
      ...
      if (!Array.isArray(data) || data.length === 0){
        console.warn('GSX: no bars from worker for tf=', tf, data);
        return;
      }
      ...
      // hooks
      if (typeof window.gsxLoadBars === 'function'){
        window.gsxLoadBars(bars, tf);
      } else if (window.pannels && typeof window.pannels.setSeries === 'function'){
        window.pannels.setSeries(bars, tf);
      } else {
        window.gsxLastBars = bars;
        console.log('GSX bars ready:', bars.slice(0,5));
      }
    }catch(e){
      console.error('GSX fetchBars failed:', e);
    }
  }

  window.addEventListener('gsx:fetch', fetchBars);
  // optional: auto fetch on load once (kept conservative)
  // document.addEventListener('DOMContentLoaded', () => setTimeout(() => window.dispatchEvent(new CustomEvent('gsx:fetch')), 200));
})();
// ===== end GSX additions =====
