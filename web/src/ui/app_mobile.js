...
// === GSX GLUE V2 ADDED ===
// (Non-invasive) live price poller + /bars loader and minimal renderer.
// This block is self-contained and won't touch your existing logic.
...
(function(){
  ...
  // ---------------- Price ----------------
  function getSavedBase(){ try{return (localStorage.getItem(LS_KEY)||'').trim();}catch(e){return '';} }
  function setSavedBase(v){ const c=String(v||'').trim().replace(/\/+$/,''); try{localStorage.setItem(LS_KEY,c);}catch(e){}; const el=$('#base')||$('#workerUrl'); if(el) el.value=c; return c; }
  function getUserBase(){
    const el=$('#base')||$('#workerUrl');
    const fromUI = el && (el.value||'').trim() || '';
    return (fromUI || getSavedBase() || '').replace(/\/+$/,'');
  }
  function normPrice(j){
    const p=Number(j?.price ?? j?.close ?? j?.last);
    if(!Number.isFinite(p)) return null;
    const iso=j?.isoTime || (typeof j?.ts==='number' ? new Date(j.ts).toISOString() : null);
    const ts = (typeof j?.ts==='number') ? j.ts : (iso ? Date.parse(iso): Date.now());
    return {price:p, ts, source:j?.source||'worker', isoTime:iso};
  }
  async function tryBase(base){
    for(const u of [`${base}/price`, `${base}/api/price`, base]){
      try{
        const r=await fetch(u,{headers:{accept:'application/json'}, cache:'no-store'});
        if(!r.ok) continue;
        const j=await r.json().catch(()=>({}));
        const m=normPrice(j); if(m) return m;
      }catch(_){}
    }
    return null;
  }
  async function fetchUnifiedPrice(){
    const bases=[];
    const user=getUserBase(); if(user) bases.push(user);
    if(window.__GSX_LAST_OK_BASE__ && !bases.includes(window.__GSX_LAST_OK_BASE__)) bases.push(window.__GSX_LAST_OK_BASE__);
    for(const d of DEFAULTS) if(!bases.includes(d)) bases.push(d);
    for(const b of bases){ const m=await tryBase(b); if(m){ window.__GSX_LAST_OK_BASE__=b; if(!user) setSavedBase(b); return m; } }
    // Fallback: stooq CSV
    try{
      const r=await fetch('https://stooq.com/q/l/?s=xauusd&f=sd2t2ohlcvn&h&e=csv', {cache:'no-store'});
      if(!r.ok) throw 0; const csv=await r.text();
      const row=(csv.trim().split(/\r?\n/)[1]||'').split(',');
      const close=Number(row[6]); if(!Number.isFinite(close)) throw 0;
      const ts=Date.parse((row[1]||'')+'T'+(row[2]||'')+'Z') || Date.now();
      return {price:close, ts, source:'stooq'};
    }catch(_){ return null; }
  }
  function updatePriceUI(meta){
    const el = document.getElementById('price') || document.getElementById('livePrice') || document.querySelector('.live-price');
    if(el && meta){ el.textContent=(Math.round(meta.price*1000)/1000).toString(); el.setAttribute('data-ts', String(meta.ts)); }
    if(typeof window.setLivePrice==='function'){ try{ window.setLivePrice(meta?.price, meta||{});}catch(e){} }
    const dt=document.getElementById('liveDateTime');
    if(dt && meta){ try{ const d=new Date(meta.isoTime || meta.ts || Date.now()); const pad=n=>String(n).padStart(2,'0'); dt.textContent=`${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())} ${pad(d.getHours())}:${pad(d.getMinutes())}:${pad(d.getSeconds())}`; }catch(e){} }
  }
  async function priceTick(){ const m=await fetchUnifiedPrice(); updatePriceUI(m); }
  function startPrice(){ if(window.__GSX_PRICE_TIMER) return; priceTick(); window.__GSX_PRICE_TIMER=setInterval(priceTick, 1000); }
  function stopPrice(){ if(window.__GSX_PRICE_TIMER){ clearInterval(window.__GSX_PRICE_TIMER); window.__GSX_PRICE_TIMER=null; } }
  document.addEventListener('visibilitychange', ()=>{ if(document.hidden) stopPrice(); else startPrice(); });

  // ---------------- Bars (/bars) ----------------
  async function loadBarsOnce(){
    try{
      const base=(getUserBase() || DEFAULTS[0]).replace(/\/+$/,'');
      const tfBtn=document.querySelector('button[data-tf].active') || document.querySelector('button[data-tf="1m"]');
      const tf=tfBtn ? (tfBtn.getAttribute('data-tf')||'1m') : '1m';
      const limEl=document.querySelector('#limitInput'); const limit=Number(limEl?.value||1200);
      const r=await fetch(`${base}/bars?tf=${encodeURIComponent(tf)}&limit=${limit}`, {headers:{accept:'application/json'}});
      if(!r.ok) throw new Error('bars '+r.status);
      const bars=await r.json();
      window.dispatchEvent(new CustomEvent('gsx:bars', {detail:{tf,limit,bars}}));
    }catch(e){ console.warn('[GSX] bars error:', e); }
  }
  // Minimal render with LightweightCharts if present
  (function(){
    function toCandleData(bars){
      return (bars||[]).map(b=>({ time: Math.floor((b.t||b.time||b.ts)/1000), open:b.o, high:b.h, low:b.l, close:b.c }));
    }
    function render(bars){
      const el=document.getElementById('chart') || document.querySelector('#tvchart, #chartRoot');
      if(!el) return;
      window.gsx=window.gsx||{};
      if(!window.gsx.lw && window.LightweightCharts){
        const chart=LightweightCharts.createChart(el,{ height:420, layout:{ background:{type:'solid',color:'#0b0f14'}, textColor:'#e6e6e6'}, rightPriceScale:{borderVisible:false}, timeScale:{borderVisible:false} });
        const series=chart.addCandlestickSeries();
        window.gsx.lw={chart,series};
      }
      if(window.gsx?.lw?.series) window.gsx.lw.series.setData(toCandleData(bars));
    }
    window.addEventListener('gsx:bars', (e)=>{ const bars=e?.detail?.bars||[]; if(bars.length) { if(typeof window.computeAndRender==='function'){ try{ window.computeAndRender(bars); return; }catch(_){ } } render(bars); } });
  })();

  // Bind fetch button if exists
  const btn=document.querySelector('#btnFetch'); if(btn && !btn.__gsxBound){ btn.__gsxBound=true; btn.addEventListener('click', loadBarsOnce); }
  // Expose small API
  window.GSX_GLUE = Object.assign(window.GSX_GLUE||{}, { startPrice, stopPrice, loadBarsOnce, setBase:setSavedBase });

  // Auto-start
  if(document.readyState==='loading'){ document.addEventListener('DOMContentLoaded', startPrice); } else { startPrice(); }
})();
