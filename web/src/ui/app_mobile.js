...
function ensureCharts(){
  ...
  const h = c.offsetHeight || parseInt(c.style.height || '0', 10);
  wrap.style.height = (h && Number.isFinite(h)) ? `${h}px` : '280px';
  c.replaceWith(wrap);
...
function setBarsOnCharts(bars){
  ...
  // لو لسبب ما الشارت مش مخلوق، نعيد إنشاؤه
  ...
  const data = bars.map(b=>({
    time: Math.floor(b.t/1000),  // lightweight-charts يريد seconds
    open: b.o,
    high: b.h,
    low:  b.l,
    close:b.c
...
async function fetchBarsAndUpdate(){
  ...
  try{
    ...
    // تحويل الوقت t لأي شكل (رقم/ثواني/ISO) إلى ms + تحويل الباقي لأرقام
    lastBars = (Array.isArray(j) ? j : []).map(b => {
      const rawT = b.t ?? b.time ?? b.timestamp ?? b.ts ?? b.isoTime ?? b.date;

      let tNum;
      if (typeof rawT === 'string') {
        tNum = Number(rawT);
        if (!Number.isFinite(tNum)) {
          tNum = Date.parse(rawT);
        }
      } else {
        tNum = Number(rawT);
      }

      // إذا فشل كل شيء، استعمل الآن حتى ما نرمي البيانات
      if (!Number.isFinite(tNum)) {
        tNum = Date.now();
      }

      // لو كان بالثواني (صغير بالنسبة للميلي ثانية) نحوله لميلي ثانية
      if (tNum < 2e12) {
        tNum *= 1000;
      }
