// === GSX Live Price Integration (non-invasive) ===
// This block adds a default Worker base URL, gold-ticks compatible /price fetch,
// and a light auto-poller that updates UI if certain IDs exist.
// It does NOT depend on your existing variables. Safe to drop-in.
...
(() => {
  ...
  function gsxGetBase() {
    try {
      const v = (localStorage.getItem('GSX_BASE_URL') || GSX_DEFAULT_BASE || '').trim();
      return v.replace(/\/+$/,''); // remove trailing slashes
    } catch (_) {
      return GSX_DEFAULT_BASE;
    }
  }
  function gsxSetBase(v) {
    try {
      localStorage.setItem('GSX_BASE_URL', String(v || '').trim());
    } catch (_) {}
  }

  async function gsxFetchPrice(base, timeoutMs = 6000) {
    base = (base || gsxGetBase()).replace(/\/+$/,'');
    ...
    for (const u of candidates) {
      try {
        const r = await withTimeout(fetch(u, { headers: { accept:'application/json' }, cache:'no-store' }), timeoutMs);
        if (!r.ok) { lastErr = new Error('HTTP ' + r.status); continue; }
        const j = await r.json();
        const price = Number(j.price ?? j.close ?? j.last);
        if (!Number.isFinite(price)) { lastErr = new Error('bad_price'); continue; }

        // Normalize to gold-ticks-like shape
        const symbol  = j.symbol || 'XAUUSD';
        const isoTime = j.isoTime || (j.date && j.time ? `${j.date}T${j.time}Z` : new Date().toISOString());
        const ts      = Number(j.ts) || Date.parse(isoTime) || Date.now();
        const source  = j.source || 'gold-ticks';

        return { ok:true, source, symbol, price, close: Number(j.close ?? price), isoTime, ts,
                 date: j.date || new Date(ts).toISOString().slice(0,10),
                 time: j.time || new Date(ts).toISOString().slice(11,19).replace('Z','') };
      } catch (e) {
        lastErr = e;
      }
    }
    throw lastErr || new Error('price_fetch_failed');
  }

  function gsxApplyPrice(meta) {
    // If the app already has a hook for live price, use it
    if (typeof window.setLivePrice === 'function') {
      try { window.setLivePrice(+meta.price, meta); } catch(_) {}
    }
    // Update default UI elements if they exist
    const $ = (id) => document.getElementById(id);
    const p = $('livePrice');  if (p) p.textContent = Number(meta.price).toFixed(3);
    const s = $('liveSource'); if (s) s.textContent = meta.source || '';
    const t = $('liveTime');   if (t) t.textContent = meta.isoTime || new Date(meta.ts).toISOString();
  }

  let __gsxTimer = null;
  async function gsxTick() {
    try {
      const j = await gsxFetchPrice();
      gsxApplyPrice(j);
    } catch (e) {
      // console.warn('[GSX] fetch failed:', e);
    }
  }
  function gsxStartLivePrice(intervalMs = 1000) {
    gsxStopLivePrice();
    gsxTick(); // immediate
    __gsxTimer = setInterval(gsxTick, intervalMs);
    document.addEventListener('visibilitychange', gsxHandleVisibility, { passive: true });
  }
  function gsxStopLivePrice() {
    if (__gsxTimer) { clearInterval(__gsxTimer); __gsxTimer = null; }
    document.removeEventListener('visibilitychange', gsxHandleVisibility);
  }
  function gsxHandleVisibility() {
    if (document.hidden) gsxStopLivePrice();
    else gsxStartLivePrice(1000);
  }

  // Expose small API if needed elsewhere
  window.GSX = Object.assign(window.GSX || {}, {
    getBase: gsxGetBase, setBase: gsxSetBase,
    fetchPrice: gsxFetchPrice,
    startLivePrice: gsxStartLivePrice, stopLivePrice: gsxStopLivePrice
  });

  // Auto-start when DOM is ready (non-invasive)
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => gsxStartLivePrice(), { once: true });
  } else {
    gsxStartLivePrice();
  }
})();
