...
(function () {
  ...
  function updateUI(meta) {
    try {
      ...
      // Fallback: if not found, try alternative IDs or create a floating pill
      let _elPrice = elPrice || document.getElementById('price') || document.getElementById('priceVal') || document.querySelector('[data-role="live-price"]');
      let elTime = document.getElementById('liveTime') || document.getElementById('time') || document.querySelector('[data-role="live-time"]');
      let elSource = document.getElementById('liveSource') || document.getElementById('source') || document.querySelector('[data-role="live-source"]');
      if (!_elPrice) {
        _elPrice = document.createElement('div');
        _elPrice.id = 'livePrice';
        _elPrice.style.cssText = 'position:fixed;top:10px;left:10px;padding:6px 10px;border-radius:10px;background:#0b0f14;color:#cde7ff;font-weight:600;z-index:9999;box-shadow:0 0 0 1px #203645 inset';
        _elPrice.textContent = '—';
        document.body.appendChild(_elPrice);
      }
